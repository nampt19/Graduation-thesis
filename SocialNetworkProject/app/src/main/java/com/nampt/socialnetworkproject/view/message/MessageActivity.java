...
import static com.nampt.socialnetworkproject.view.search.SearchActivity.LAUNCH_PROFILE_ACTIVITY;
...
public class MessageActivity extends AppCompatActivity {
    ...
    String roomName;
    boolean isRoom, isPartnerBlocked;
        ...
        webSocketClientSingle = new WebSocketClient(uri) {
            ...
            public void onTextReceived(String s) {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            if (s.equals("2peopleblocked")) {
                                isPartnerBlocked = true;
                                Toast.makeText(MessageActivity.this, "Không thể trò chuyện, 2 người đã chặn nhau !", Toast.LENGTH_SHORT).show();
                                return;
                            }
                    ...
                    .enqueue(new Callback<ListMessageResponse>() {
                        ...
                        public void onResponse(Call<ListMessageResponse> call, Response<ListMessageResponse> response) {
                            ...
                            if (response.body().getCode() == 1000) {
                                ...
                                if (partnerId != 0 && !messageList.isEmpty()) {
                                    for (MessageRow row : messageList) {
                                        if (row.getSender().getId() == partnerId) {
                                            isPartnerBlocked = row.getSender().isBlock();
                                            break;
                                        }
                                    }
                                }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == LAUNCH_PROFILE_ACTIVITY) {
            if (resultCode == Activity.RESULT_CANCELED) {
                if (data == null) return;
                if (partnerId == data.getIntExtra("idUserBlock", 0)) {
                    isPartnerBlocked = data.getBooleanExtra("isUserBlockFromMessageActivity", false);
                    Log.e("kkkk", isPartnerBlocked + "--" + partnerId);
                }
            }
        }
    }
